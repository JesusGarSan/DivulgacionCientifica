...
#@st.cache_data
def Doppler_audio(input_file, output_file, delta_f, v_e=0, c=340):
    ...
    audio_data_alterado = librosa.effects.time_stretch(audio_data_alterado, rate= 1 + rate )
...
def dashboard_parametros():
    ...
    velocidad_emisor = column[0].number_input('Velocidad (m/s)', value= 10.0, min_value=-400.0, max_value=+400.0, step= 1.0, )
    ...
    velocidad_receptor = column[1].number_input('Velocidad (m/s)', value= 0.0, min_value=-400.0, max_value=+400.0, step= 1.0, key='velocidad_receptor' )
...
def audio_to_Doppler(f_receptor, f_emisor, v_e=0, c=340):
    column = st.columns(2)
    ...
    if archivo and seleccion ==None:
        ...
        archivo = 'Simulaciones/Doppler/'+seleccion
        filename = seleccion
        
    if archivo or seleccion!=None:
        input_file=archivo
        output_file = 'Simulaciones/Doppler/Doppler_test.wav'
